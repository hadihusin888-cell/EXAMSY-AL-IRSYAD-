...
const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<"SESSIONS" | "STUDENTS" | "ROOMS">(
    "SESSIONS",
  );
  ...
  const [sessionToToggleStatus, setSessionToToggleStatus] =
  ...
  useEffect(() => {
    if (showAddSession || sessionToEdit) {
      setModalIsActive(!!sessionToEdit?.isActive);
    }
  ...
  const [showBulkDeleteSessionsConfirm, setShowBulkDeleteSessionsConfirm] =
    useState(false);
  ...
  const [roomToViewStudents, setRoomToViewStudents] = useState<Room | null>(
    null,
  );
  ...
  const filteredStudents = useMemo(() => {
    return students.filter((s) => {
      ...
      const matchesRoom =
        roomFilter === "ALL" ||
        String(s.roomId || "").trim() === String(roomFilter).trim();
  ...
  const toggleSelectAllVisible = () => {
    const allVisibleSelected =
      ...
      filteredStudents.every((s) => selectedNis.includes(String(s.nis)));
  ...
  const toggleSelectAllSessions = () => {
    const allVisibleSelected =
      ...
      sortedSessions.every((s) => selectedSessionIds.includes(s.id));
    if (allVisibleSelected) {
    ...
    reader.onload = async (event) => {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (nis && name) {
          ...
          if (
            Object.values(StudentStatus).includes(inputStatus as StudentStatus)
          ) {
  ...
  const getStatusBadge = (status: StudentStatus) => {
    switch (status) {
      ...
      case StudentStatus.SEDANG_UJIAN:
        return (
          <span className="bg-amber-100 text-amber-600 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider animate-pulse">
            Sedang Ujian
          </span>
        );
      ...
      case StudentStatus.BLOKIR:
        return (
          <span className="bg-red-100 text-red-600 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider">
            Blokir
          </span>
        );
      default:
    ...
    <div className="flex flex-col h-screen overflow-hidden bg-slate-50 font-sans">
      <header className="bg-white border-b border-slate-200 px-6 md:px-10 py-5 flex items-center justify-between shrink-0 z-50">
        <div className="flex items-center gap-4 md:gap-10">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 md:w-10 md:h-10 bg-indigo-600 rounded-lg md:rounded-xl flex items-center justify-center text-white font-black italic shadow-lg shadow-indigo-200">
              E
            </div>
            <h1 className="text-lg md:text-xl font-black text-slate-900 tracking-tight hidden sm:block">
              Examsy Super Admin
            </h1>
          </div>
          <nav className="flex bg-slate-100 p-1 rounded-xl md:rounded-2xl">
            {(["SESSIONS", "STUDENTS", "ROOMS"] as const).map((tab) => (
              <button
                ...
                className={`px-3 md:px-6 py-1.5 md:py-2 rounded-lg md:rounded-xl text-[10px] md:text-xs font-black uppercase tracking-widest transition-all ${activeTab === tab ? "bg-white text-indigo-600 shadow-sm" : "text-slate-500 hover:text-slate-800"}`}
              >
                {tab === "SESSIONS"
                  ? "Ujian"
                  : tab === "STUDENTS"
        ...
        <div className="flex items-center gap-3 md:gap-6">
          <div className="flex items-center gap-2 md:gap-3 bg-slate-50 px-3 md:px-4 py-2 rounded-xl md:rounded-2xl border border-slate-100">
            <div
              className={`w-1.5 h-1.5 md:w-2 md:h-2 rounded-full ${isSyncing ? "bg-amber-500 animate-pulse" : "bg-emerald-500"}`}
            ></div>
            <span className="text-[9px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest">
              {isSyncing ? "Sinkronisasi..." : "Terhubung Cloud"}
            </span>
          </div>
          <button
            onClick={onLogout}
            className="text-slate-400 hover:text-red-600 font-bold text-xs md:text-sm"
          >
            Logout
          </button>
        </div>
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 gap-4">
              <div className="flex-1">
                ...
                <p className="text-slate-400 font-bold text-[10px] uppercase tracking-widest mt-2">
                  Jadwal & Soal Terintegrasi Cloud
                </p>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
                <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200/60 shadow-inner">
                  {(["ALL", "ACTIVE", "DRAFT"] as const).map((filter) => (
                    <button
                      ...
                      className={`px-3.5 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all duration-200 ${
                        sessionStatusFilter === filter
                ...
                <button
                  ...
                  className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl transition-all active:scale-95"
                >
                  {" "}
                  + Sesi Baru{" "}
                </button>
            ...
            {selectedSessionIds.length > 0 && (
              <div className="mb-10 bg-indigo-600 text-white px-8 py-4 rounded-3xl flex items-center justify-between animate-in slide-in-from-top-6 shadow-2xl z-[60] sticky top-6">
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    onClick={() => setShowBulkDeleteSessionsConfirm(true)}
                    className="px-6 py-3 bg-red-500 hover:bg-red-600 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all shadow-lg"
                  >
                    Hapus Masal
                  </button>
                </div>
                    ...
                    <div key={date} className="space-y-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {sessionsInDate.map((session) => (
                          <div
                            key={session.id}
                            className={`bg-white p-7 rounded-[2.5rem] border transition-all group relative overflow-hidden ${selectedSessionIds.includes(session.id) ? "border-indigo-500 ring-2 ring-indigo-500/20 shadow-xl" : "border-slate-200 shadow-sm hover:shadow-xl"}`}
                          >
                            <div className="flex justify-between items-start mb-4">
                              ...
                              {session.isActive ? (
                                <button
                                  type="button"
                                  onClick={() => setSessionToToggleStatus(session)}
                                  className="px-3.5 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest transition-all border bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-100 flex items-center gap-1.5 shadow-sm hover:scale-[1.03] active:scale-95"
                                >
                                ...
                                <button
                                  type="button"
                                  onClick={() => setSessionToToggleStatus(session)}
                                  className="px-3.5 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest transition-all border bg-slate-100 text-slate-600 border-slate-200 hover:bg-slate-200 flex items-center gap-1.5 hover:scale-[1.03] active:scale-95 whitespace-nowrap"
                                >
                            ...
                            <div className="flex gap-2 mb-8">
                              <span className="bg-slate-50 text-slate-500 text-[9px] font-black px-3 py-1.5 rounded-lg uppercase tracking-wider border border-slate-100">
                                Kls {session.class}
                              </span>
                              <span className="bg-indigo-50 text-indigo-600 text-[9px] font-black px-3 py-1.5 rounded-lg uppercase tracking-wider border border-indigo-100">
                                PIN: {session.pin}
                              </span>
                              <span className="bg-slate-50 text-slate-500 text-[9px] font-black px-3 py-1.5 rounded-lg uppercase tracking-wider border border-slate-100">
                            ...
                            <div className="flex gap-2">
                              <button
                                ...
                                className="flex-1 bg-slate-900 text-white h-12 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-black transition-all"
                              >
                                Lihat Soal
                              </button>
                              <button
                                onClick={() => setSessionToEdit(session)}
                                className="w-12 h-12 flex items-center justify-center bg-indigo-50 text-indigo-600 rounded-2xl border border-indigo-100 hover:bg-indigo-600 hover:text-white transition-all"
                              >
                                <svg
                                  xmlns="http://www.w3.org/2000/svg"
                                  className="h-4 w-4"
                              ...
                              <button
                                ...
                                className="w-12 h-12 flex items-center justify-center bg-red-50 text-red-500 rounded-2xl border border-red-100 hover:bg-red-600 hover:text-white transition-all"
                              >
                                <svg
                                  xmlns="http://www.w3.org/2000/svg"
                                  className="h-4 w-4"
          ...
          <div className="max-w-7xl mx-auto pb-32">
            <div className="flex flex-col lg:flex-row justify-between lg:items-end mb-10 gap-6">
              <div>
                ...
                <p className="text-slate-400 font-bold text-[10px] uppercase tracking-widest mt-2">
                  Database Terintegrasi ({students.length})
                </p>
              </div>
              <div className="flex flex-wrap gap-3">
                <div className="flex gap-2">
                  <input
                    ...
                    className="w-full lg:w-48 pl-6 pr-6 py-3.5 bg-white border border-slate-200 rounded-2xl text-sm outline-none focus:border-indigo-500 shadow-sm"
                  />
                  <select
                    value={roomFilter}
                    onChange={(e) => setRoomFilter(e.target.value)}
                    className="bg-white border border-slate-200 rounded-2xl px-4 text-xs font-black uppercase text-slate-500 outline-none focus:border-indigo-500 shadow-sm"
                  >
                ...
                <button
                  onClick={() => setStudentToAdd(true)}
                  className="bg-indigo-600 text-white px-5 py-3.5 rounded-2xl font-black text-[10px] uppercase shadow-lg active:scale-95 transition-all"
                >
                  {" "}
                  + Siswa{" "}
                </button>
                <button
                  ...
                  className="bg-emerald-600 text-white px-5 py-3.5 rounded-2xl font-black text-[10px] uppercase transition-all shadow-lg active:scale-95"
                >
                  Template
                </button>
                <button
                  onClick={() => fileInputRef.current?.click()}
                  className="bg-slate-900 text-white px-5 py-3.5 rounded-2xl font-black text-[10px] uppercase transition-all shadow-lg active:scale-95"
                >
                  Impor CSV
                </button>
                <input
              ...
              <div className="mb-6 bg-indigo-600 text-white px-8 py-4 rounded-3xl flex items-center justify-between animate-in slide-in-from-top-6 shadow-2xl z-[60] sticky top-6">
                ...
                <div className="flex flex-wrap gap-3">
                  <button
                    ...
                    className="bg-white/10 hover:bg-white text-white hover:text-indigo-600 px-5 py-2.5 rounded-xl font-black text-[10px] uppercase border border-white/30 transition-all shadow-sm"
                  >
                    Ubah Ruang
                  </button>
                  <button
                  ...
                  <button
                    ...
                    className="bg-red-500 hover:bg-red-600 text-white px-5 py-2.5 rounded-xl font-black text-[10px] uppercase shadow-md transition-all"
                  >
                    Hapus
                  </button>
                  <button
                    onClick={() => setSelectedNis([])}
                    className="text-white/70 hover:text-white font-black text-[10px] uppercase underline ml-2 transition-all"
                  >
                    Batal
                  </button>
                </div>
                        ...
                        <input
                          ...
                          checked={
                            ...
                            filteredStudents.every((s) =>
                              selectedNis.includes(String(s.nis)),
                            )
                          }
                        />
                      </th>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {filteredStudents.length === 0 ? (
                      <tr>
                        <td
                          colSpan={7}
                          className="px-8 py-20 text-center text-slate-300 font-black uppercase tracking-widest text-xs"
                        >
                        ...
                        >
                          <td className="px-6 py-5 text-center">
                            <input
                              ...
                              checked={selectedNis.includes(
                                String(student.nis),
                              )}
                              onChange={() =>
                                setSelectedNis((prev) =>
                                  prev.includes(String(student.nis))
                                    ? prev.filter(
                                        (n) => n !== String(student.nis),
                                      )
                                    : [...prev, String(student.nis)],
                                )
                              }
                            />
                          </td>
                          <td className="px-8 py-5 font-mono text-indigo-600 text-[11px] font-black uppercase tracking-tight">
                            {student.nis}
                          </td>
                          <td className="px-8 py-5 font-black text-slate-800 tracking-tight uppercase text-xs">
                            {student.name}
                          </td>
                          <td className="px-8 py-5 text-[11px] font-bold text-slate-500 uppercase">
                            Kls {student.class}
                          </td>
                          <td className="px-8 py-5 text-[11px] font-black text-indigo-900 uppercase">
                            {rooms.find((r) => r.id === student.roomId)?.name ||
                              "-"}
                          </td>
          ...
          <div className="max-w-7xl mx-auto">
            <div className="flex justify-between items-end mb-10">
              <div>
                ...
                <p className="text-slate-400 font-bold text-[10px] uppercase tracking-widest mt-2">
                  Total {rooms.length} Ruang Proktor Terdaftar
                </p>
              </div>
              <button
                onClick={() => setShowAddRoom(true)}
                className="bg-indigo-600 text-white px-8 py-4 rounded-2xl font-black text-xs uppercase shadow-xl transition-all active:scale-95"
              >
                {" "}
                + Tambah Ruang{" "}
              </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {rooms.map((room) => {
                const roomParticipants = students.filter(
                  (s) => s.roomId === room.id,
                );
                return (
                  <div
                    key={room.id}
                    className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm hover:shadow-xl transition-all group flex flex-col h-full"
                  >
                    <div className="mb-4">
                      <h3 className="text-xl font-black text-slate-900 uppercase tracking-tighter leading-none">
                        {room.name}
                      </h3>
                      <p className="text-indigo-600 font-black text-[9px] uppercase tracking-widest mt-1.5">
                        {roomParticipants.length} / {room.capacity} Siswa
                        Terdaftar
                      </p>
                    </div>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-900/60 backdrop-blur-md p-4 animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-2xl h-[80vh] flex flex-col rounded-[3.5rem] shadow-2xl relative animate-in zoom-in-95 overflow-hidden">
            <header className="p-8 border-b border-slate-100 flex items-center justify-between shrink-0">
              <div>
                <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tighter leading-none">
                  {roomToViewStudents.name}
                </h3>
                <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest mt-2">
                  Daftar Peserta yang Ditempatkan
                </p>
              </div>
              <button
                onClick={() => setRoomToViewStudents(null)}
            ...
            <div className="flex-1 overflow-auto p-8 custom-scrollbar">
              {students.filter((s) => s.roomId === roomToViewStudents.id)
                .length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {students
                    .filter((s) => s.roomId === roomToViewStudents.id)
                    ...
                    .map((s) => (
                      ...
                      >
                        <div className="overflow-hidden">
                          <p className="text-[10px] font-black text-indigo-600 leading-none mb-1.5 uppercase tracking-tight">
                            {s.nis}
                          </p>
                          ...
                          <p className="text-[9px] font-bold text-slate-400 mt-1 uppercase">
                            Pass:{" "}
                            <span className="text-indigo-500 font-black normal-case">
                              {s.password || "-"}
                            </span>
                          </p>
                        </div>
            ...
            <footer className="p-8 bg-slate-50 border-t border-slate-100 text-center shrink-0">
              <button
                onClick={() => setRoomToViewStudents(null)}
                className="px-10 py-4 bg-slate-900 hover:bg-black text-white rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] transition-all"
              >
                Tutup Daftar
              </button>
            </footer>
      ...
      {showBulkRoomModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-900/60 backdrop-blur-md p-4 animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-md p-10 rounded-[3.5rem] shadow-2xl animate-in zoom-in-95">
            ...
            <p className="text-slate-500 text-[10px] font-bold uppercase mb-10 leading-relaxed">
              Anda akan memindahkan{" "}
              <span className="text-indigo-600 font-black">
                {selectedNis.length} siswa
              </span>{" "}
              sekaligus ke unit ruang:
            </p>
            <div className="grid grid-cols-1 gap-3 max-h-[40vh] overflow-y-auto pr-2 custom-scrollbar">
              <button
                onClick={() => handleBulkUpdate({ roomId: "" })}
                className="w-full bg-slate-100 hover:bg-slate-200 text-slate-500 py-5 rounded-2xl font-black text-[11px] uppercase transition-all flex items-center justify-center gap-3"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="h-5 w-5"
              ...
              {rooms.map((r) => (
                <button
                  key={r.id}
                  onClick={() => handleBulkUpdate({ roomId: r.id })}
                  className="w-full bg-indigo-50 hover:bg-indigo-600 group text-indigo-600 hover:text-white py-5 rounded-2xl font-black text-[11px] uppercase transition-all flex items-center justify-between px-6 border border-indigo-100"
                >
                  <span>{r.name}</span>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
            ...
            <button
              ...
              className="w-full mt-8 text-slate-400 font-bold py-2 text-[10px] uppercase tracking-widest hover:text-slate-600 transition-colors"
            >
              Batalkan Pemindahan
            </button>
          </div>
        ...
        <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-sm z-[100] flex items-center justify-center p-6">
          <div className="bg-white w-full max-w-md rounded-[3rem] p-10 text-center animate-in zoom-in-95 duration-200">
            ...
            <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tight mb-2">
              Hapus {selectedSessionIds.length} Sesi?
            </h3>
            <p className="text-slate-500 text-sm font-bold mb-8">
              Tindakan ini tidak dapat dibatalkan. Semua data terkait sesi ini
              akan dihapus permanen.
            </p>
            <div className="flex gap-3">
              <button
                onClick={() => setShowBulkDeleteSessionsConfirm(false)}
        ...
        <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-slate-900/70 backdrop-blur-md p-4 animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-5xl h-[90vh] flex flex-col rounded-[3.5rem] shadow-2xl relative animate-in zoom-in-95 overflow-hidden border border-white/20">
            <header className="p-8 md:p-10 border-b border-slate-100 flex items-center justify-between shrink-0 bg-white z-20">
              <div className="flex-1">
                ...
                <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest mt-3">
                  Pratinjau File PDF Soal
                </p>
              </div>
              <button
                onClick={() => setSessionToView(null)}
                className="p-3 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-2xl transition-all ml-4"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="h-8 w-8"
              ...
              <div className="w-full h-full flex flex-col bg-slate-900">
                {sessionToView.pdfUrl ? (
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center text-center p-10">
                    <div className="w-20 h-20 bg-white/5 rounded-[2rem] flex items-center justify-center mb-6 text-white/20">
                      ...
                      >
                        <path
                          ...
                          strokeLinejoin="round"
                          strokeWidth={2}
                          d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.268 15c-.77 1.333.192 3 1.732 3z"
                        />
                      </svg>
                    </div>
                    ...
                    <p className="text-white/40 text-xs mt-2 font-bold uppercase tracking-widest max-w-xs">
                      Silakan lampirkan link Google Drive PDF di menu 'Edit
            ...
            <footer className="p-8 bg-white border-t border-slate-100 flex justify-end shrink-0 z-20">
              <button
                onClick={() => setSessionToView(null)}
                className="px-12 py-5 bg-slate-900 hover:bg-black text-white rounded-3xl font-black text-xs uppercase tracking-[0.2em] transition-all shadow-xl active:scale-95"
              >
                Tutup Jendela
              </button>
            </footer>
            ...
            <form
              onSubmit={async (e) => {
                ...
                const ok = await onAction(
                  sessionToEdit ? "UPDATE_SESSION" : "ADD_SESSION",
                  data,
                );
                if (ok) {
                  setShowAddSession(false);
            ...
            >
              <div className="space-y-2">
                ...
                <input
                  ...
                  required
                  placeholder="Contoh: MATEMATIKA PAT"
                  className="w-full px-6 py-4.5 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold outline-none uppercase focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all"
                />
              </div>
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  required
                  className="w-full px-6 py-4.5 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <select
                    name="class"
                    defaultValue={sessionToEdit?.class || "7"}
                    className="w-full px-5 py-4.5 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold outline-none focus:border-indigo-500 transition-all appearance-none"
                  >
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    maxLength={6}
                    placeholder="ABCD"
                    className="w-full px-5 py-4.5 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-black text-center uppercase tracking-widest outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all"
                  />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <input
                  name="duration"
                  type="number"
                  defaultValue={sessionToEdit?.durationMinutes || 90}
                  required
                  className="w-full px-5 py-4.5 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold outline-none focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all"
                />
              </div>

              <div className="space-y-2">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">
                  PDF URL (Sematkan Google Drive)
                </label>
                <input
                  name="pdfUrl"
              ...
              <div className="flex items-center justify-between p-5 bg-slate-50 border border-slate-200 rounded-3xl transition-all">
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {modalIsActive ? (
                      <span className="inline-flex items-center gap-1 bg-emerald-50 text-emerald-700 border border-emerald-100 text-[9px] font-black uppercase px-2 py-0.5 rounded-lg">
                        <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
                        Aktif
                      </span>
                    ) : (
                      <span className="inline-flex items-center gap-1 bg-slate-100 text-slate-500 border border-slate-200 text-[9px] font-black uppercase px-2 py-0.5 rounded-lg">
                        <span className="w-1.5 h-1.5 rounded-full bg-slate-400"></span>
                        Draft
                      </span>
                    )}
              ...
              <div className="pt-8 flex flex-col gap-3">
                <button
                  ...
                  className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-400 text-white py-5 rounded-[2rem] font-black uppercase text-xs tracking-[0.2em] shadow-xl shadow-indigo-100 transition-all flex items-center justify-center gap-3 active:scale-[0.98]"
                >
                  {isProcessing && (
                ...
                <button
                  ...
                  className="w-full text-slate-400 py-2 font-bold uppercase text-[10px] tracking-widest hover:text-slate-600 transition-colors"
                >
                  Batal
                </button>
              </div>
      ...
      {sessionToToggleStatus && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-900/60 backdrop-blur-md p-4 animate-in fade-in duration-200">
          <div
            className={`bg-white w-full max-w-md p-10 rounded-[3.5rem] shadow-2xl animate-in zoom-in-95 duration-200 border-t-8 ${sessionToToggleStatus.isActive ? "border-amber-500" : "border-emerald-600"}`}
          >
            <div className="text-center">
              <div
                className={`w-16 h-16 mx-auto mb-6 rounded-3xl flex items-center justify-center ${sessionToToggleStatus.isActive ? "bg-amber-50 text-amber-500" : "bg-emerald-50 text-emerald-500"}`}
              >
                {sessionToToggleStatus.isActive ? (
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="h-8 w-8"
                    fill="none"
              ...
              <p className="text-slate-500 text-xs font-bold uppercase mb-8 leading-relaxed px-4">
                {sessionToToggleStatus.isActive
                  ? `Anda akan menonaktifkan sesi "${sessionToToggleStatus.name}". Siswa kelas ${sessionToToggleStatus.class} tidak akan bisa login lagi untuk sesi ini.`
                  : `Sesi "${sessionToToggleStatus.name}" akan diaktifkan. Siswa kelas ${sessionToToggleStatus.class} akan diizinkan login dan memulai ujian menggunakan PIN.`}
              </p>
              ...
              <div className="flex flex-col gap-3">
                <button
                  disabled={isProcessing}
                  onClick={async () => {
                    const ok = await onAction("UPDATE_SESSION", {
                      ...sessionToToggleStatus,
                      isActive: !sessionToToggleStatus.isActive,
                    });
                    if (ok) setSessionToToggleStatus(null);
                  }}
                  className={`w-full py-4.5 rounded-2xl font-black text-xs uppercase shadow-xl transition-all ${sessionToToggleStatus.isActive ? "bg-amber-500 hover:bg-amber-600 text-white" : "bg-emerald-600 hover:bg-emerald-700 text-white"}`}
                >
                  {isProcessing
                    ? "Memproses..."
                    : sessionToToggleStatus.isActive
                ...
                <button
                  type="button"
                  onClick={() => setSessionToToggleStatus(null)}
                  disabled={isProcessing}
                  className="w-full text-slate-400 font-bold py-2 text-[10px] uppercase tracking-widest hover:text-slate-600 transition-colors"
                >
                  Batal
                </button>
              </div>
      ...
      {sessionToDelete && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-900/60 backdrop-blur-md p-4 animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-sm p-10 rounded-[3rem] shadow-2xl animate-in zoom-in-95 duration-200 text-center border-t-8 border-red-600">
            ...
            <p className="text-slate-500 text-[11px] font-bold uppercase mb-8 leading-relaxed px-4">
              Sesi ujian ini akan dihapus secara permanen dari database.
            </p>
            <div className="flex flex-col gap-3">
              <button
                disabled={isProcessing}
                onClick={async () => {
                  const ok = await onAction("DELETE_SESSION", {
                    id: sessionToDelete,
                  });
              ...
              <button
                ...
                disabled={isProcessing}
                className="w-full text-slate-400 font-bold py-2 text-[10px] uppercase tracking-widest transition-colors"
              >
                Batal
              </button>
            </div>
          ...
          <div className="bg-white w-full max-sm p-10 rounded-[3rem] shadow-2xl animate-in zoom-in-95 duration-200 text-center border-t-8 border-red-600">
            ...
            <div className="flex flex-col gap-3">
              <button
                disabled={isProcessing}
                onClick={async () => {
                  const ok = await onAction("DELETE_STUDENT", {
                    nis: studentToDelete.nis,
                  });
                  if (ok) setStudentToDelete(null);
                }}
                className="w-full bg-red-600 hover:bg-red-700 text-white py-4 rounded-2xl font-black text-xs uppercase shadow-xl transition-all"
              >
              ...
              <button
                ...
                disabled={isProcessing}
                className="w-full text-slate-400 font-bold py-2 text-[10px] uppercase"
              >
                Batal
              </button>
            </div>
          ...
          <div className="bg-white w-full max-w-xl p-8 md:p-12 rounded-[3.5rem] shadow-2xl relative animate-in zoom-in-95">
            ...
            <form
              onSubmit={async (e) => {
                ...
                const ok = await onAction(
                  studentToEdit ? "UPDATE_STUDENT" : "ADD_STUDENT",
                  data,
                );
                if (ok) {
                  setStudentToEdit(null);
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="space-y-2">
                  ...
                  <select
                    name="class"
                    defaultValue={studentToEdit?.class || "7"}
                    className="w-full px-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold outline-none focus:border-indigo-500 appearance-none transition-all"
                  >
                    <option value="7">Kelas 7</option>
                    <option value="8">Kelas 8</option>
                    <option value="9">Kelas 9</option>
                ...
                <div className="space-y-2">
                  ...
                  <select
                    name="roomId"
                    defaultValue={studentToEdit?.roomId || ""}
                    className="w-full px-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold outline-none focus:border-indigo-500 appearance-none transition-all"
                  >
                ...
                <div className="space-y-2">
                  ...
                  <select
                    name="status"
                    defaultValue={
                      studentToEdit?.status || StudentStatus.BELUM_MASUK
                    }
                    className="w-full px-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold outline-none focus:border-indigo-500 appearance-none transition-all"
                  >
                    {Object.values(StudentStatus).map((s) => (
              ...
              <div className="pt-8 flex flex-col gap-3">
                <button
                  type="submit"
                  disabled={isProcessing}
                  className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-400 text-white py-5 rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl shadow-indigo-100 transition-all flex items-center justify-center gap-3 active:scale-[0.98]"
                >
                  {isProcessing && (
                    <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>
                  )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-md p-8 md:p-10 rounded-[3rem] shadow-2xl relative animate-in zoom-in-95">
            ...
            <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tighter mb-8">
              {roomToEdit ? "Edit Ruang" : "Ruang Baru"}
            </h3>
            <form
              onSubmit={async (e) => {
                e.preventDefault();
                ...
                const ok = await onAction(
                  roomToEdit ? "UPDATE_ROOM" : "ADD_ROOM",
                  data,
                );
                if (ok) {
                  setShowAddRoom(false);
            ...
            >
              <div className="space-y-1">
                ...
                <input
                  ...
                  required
                  className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold outline-none uppercase focus:border-indigo-500 transition-all"
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  required
                  className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-bold outline-none focus:border-indigo-500 transition-all"
              ...
              <button
                type="submit"
                disabled={isProcessing}
                className="w-full bg-indigo-600 text-white py-5 rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl transition-all active:scale-95 mt-4"
              >
                {isProcessing
                  ? "Menyimpan..."
                  : roomToEdit
      ...
      {roomToDelete && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-sm p-10 rounded-[3rem] shadow-2xl animate-in zoom-in-95 duration-200 text-center border-t-8 border-red-600">
            ...
            <p className="text-slate-500 text-[11px] font-bold uppercase mb-8 leading-relaxed px-4">
              Hapus ruang{" "}
              <span className="text-red-600">{roomToDelete.name}</span> secara
              permanen.
            </p>
            <div className="flex flex-col gap-3">
              <button
                disabled={isProcessing}
                onClick={async () => {
                  const ok = await onAction("DELETE_ROOM", {
                    id: roomToDelete.id,
                  });
                  if (ok) setRoomToDelete(null);
                }}
                className="w-full bg-red-600 hover:bg-red-700 text-white py-4 rounded-2xl font-black text-xs uppercase shadow-xl transition-all"
              >
                Hapus
